...
% PARAMETERS, 1 MONTH IS ONE UNIT
    ...
    param0.delta_grid = [0.008 0.009 0.01];
...
for loop_param  = 1:3
        ...
        % OTHER PARAMETERS
        param.delta = param0.delta_grid(loop_param);
    ...
    % SOLVE THE SURPLUS
        ...
        seval = ['save sol_betrand_01_' int2str(loop_param) '.mat param sol'];
        eval(seval)

    % STATIONARY DISTRIBUTION
        solve_stat_distribution_p0distr_bertrand
        seval = ['save sol_stat_betrand_01_' int2str(loop_param) '.mat param sol'];
        eval(seval);

    % SIMULATIONS
%      seval = ['load sol_stat_betrand_01_' int2str(loop_param) '.mat'];
%      eval(seval);
       simulations_p0distr_bertrand
        seval = ['save simul_betrand_01_' int2str(loop_param) '.mat param sol simul'];
        eval(seval);
