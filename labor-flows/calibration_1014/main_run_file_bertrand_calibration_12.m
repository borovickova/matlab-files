...
% PARAMETERS, 1 MONTH IS ONE UNIT
    ...
    param0.p0_sigma_grid = [0.05 0.15 0.2];
...
for loop_param  = 1:1
        ...
        % OTHER PARAMETERS
        param.p0_sigma = param0.p0_sigma_grid(loop_param);        
        param.rho     = 0.004;
        param.muL     = -param.muH;
        param.II      = 5 ;
    ...
    % SOLVE THE SURPLUS
        ...
        seval = ['save sol_betrand_12_' int2str(loop_param) '.mat param sol'];
        eval(seval)

    % STATIONARY DISTRIBUTION
        solve_stat_distribution_p0distr_bertrand
        seval = ['save sol_stat_betrand_12_' int2str(loop_param) '.mat param sol'];
        eval(seval);

    % SIMULATIONS

        simulations_p0distr_bertrand
        seval = ['save simul_betrand_12_' int2str(loop_param) '.mat param sol simul'];
        eval(seval);
