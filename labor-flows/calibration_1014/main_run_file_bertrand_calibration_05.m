...
% PARAMETERS, 1 MONTH IS ONE UNIT
    ...
    param0.gamX_grid = [0.19 0.2 0.27];
...
for loop_param  = 1:3
        ...
        % OTHER PARAMETERS
        param.gamX = param0.gamX_grid(loop_param);        
        param.rho     = 0.004;
        param.muL     = -param.muH;
        param.II      = 5 ;
    ...
    % SOLVE THE SURPLUS
        ...
        seval = ['save sol_betrand_05_' int2str(loop_param) '.mat param sol'];
        eval(seval)

    % STATIONARY DISTRIBUTION
        solve_stat_distribution_p0distr_bertrand
        seval = ['save sol_stat_betrand_05_' int2str(loop_param) '.mat param sol'];
        eval(seval);

    % SIMULATIONS
        simulations_p0distr_bertrand
        seval = ['save simul_betrand_05_' int2str(loop_param) '.mat param sol simul'];
        eval(seval);
