...
% PARAMETERS, 1 MONTH IS ONE UNIT
    ...
    param0.psi_grid = [0.29 0.31 0.35];
...
for loop_param  = 1:3
        ...
        % OTHER PARAMETERS
        param.psi = param0.psi_grid(loop_param);        
        param.rho     = 0.004;
        param.muL     = -param.muH;
        param.II      = 5 ;
    ...
    % SOLVE THE SURPLUS
        ...
        seval = ['save sol_betrand_09_' int2str(loop_param) '.mat param sol'];
        eval(seval)

    % STATIONARY DISTRIBUTION
        solve_stat_distribution_p0distr_bertrand
        seval = ['save sol_stat_betrand_09_' int2str(loop_param) '.mat param sol'];
        eval(seval);

    % SIMULATIONS
        simulations_p0distr_bertrand
        seval = ['save simul_betrand_09_' int2str(loop_param) '.mat param sol simul'];
        eval(seval);
