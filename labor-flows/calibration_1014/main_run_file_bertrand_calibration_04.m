...
for loop_param  = 3:3
        ...
        % OTHER PARAMETERS
        param.b = param0.b_grid(loop_param);        
        param.rho     = 0.004;
        param.muL     = -param.muH;
        param.II      = 5 ;
    ...
    % SOLVE THE SURPLUS
        ...
        seval = ['save sol_betrand_04_' int2str(loop_param) '.mat param sol'];
        eval(seval)

    % STATIONARY DISTRIBUTION
        solve_stat_distribution_p0distr_bertrand
        seval = ['save sol_stat_betrand_04_' int2str(loop_param) '.mat param sol'];
        eval(seval);

    % SIMULATIONS
%          seval = ['load sol_stat_betrand_04_' int2str(loop_param) '.mat'];
%         eval(seval);
        simulations_p0distr_bertrand
        seval = ['save simul_betrand_04_' int2str(loop_param) '.mat param sol simul'];
        eval(seval);
